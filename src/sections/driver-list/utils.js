...
export const handleExportExcel = (resaData) => {
  ...
  const worksheetData = [
    ...
    ...resaData.map((row) => [row.driver_id, row.name]),
  ];
  ...
  XLSX.writeFile(workbook, `drivers${Date.now()}.xlsx`);
};
...
export const handleExportPdf = (resaData) => {
  ...
  // Define the columns
  const columns = [
    { header: 'Driver_id', dataKey: 'driver_id' },
    { header: 'Driver_name', dataKey: 'name' },
  ];
